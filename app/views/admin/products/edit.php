    ...
    <div class="row">
        <div class="col-lg-6 col-md-12 d-flex flex-column gap-3">
            ...
            <div class="form-group">
                ...
                <input value="<?= $product['price_promotion'] ?? '' ?>" name="price_promotion" type="text" class="form-control">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-select" name="category_id" id="">
                    <?php foreach ($allCategory as $key => $category) : ?>
                        <option <?= $product['category_id'] == $category['id'] ? 'selected' : false ?> value="<?= $category['id'] ?>"><?= $category['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-select" name="brand_id" id="">
                    <?php foreach ($allBrand as $key => $brand) : ?>
                        <option <?= $product['brand_id'] == $brand['id'] ? 'selected' : false ?> value="<?= $brand['id'] ?>"><?= $brand['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-select" name="active" id="">
                    <option <?= $product['active'] == 0 ? 'selected' : false ?> value="0">Chưa kích hoạt</option>
                    <option <?= $product['active'] == 1 ? 'selected' : false ?> value="1">Kích hoạt</option>
                </select>
            </div>
        </div>
    </div>
